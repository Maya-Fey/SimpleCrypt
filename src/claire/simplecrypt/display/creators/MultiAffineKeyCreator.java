...
public class MultiAffineKeyCreator
	   extends KeyCreatorPanel<MultiAffineKey>
	...
	public MultiAffineKey extract()
	{
		int i = 0,
		    next = -1,
		    len = shifts.length();
		final int alen = alphabet.getLen();
		...
		while(len-- > 0) {
			...
			add[i  ] = f.getShift();
			mul[i  ] = mula;
			inv[i++] = MathHelper.modular_inverse(mula, alen);
		}
		return new MultiAffineKey(this.alphabet, add, mul, inv);
	}
						 ...
						 implements ActionListener
	{
		...
		private final MultiAffineKeyCreator owner;
		private final int pos;
		
		public AffineField(MultiAffineKeyCreator owner, int pos)
		{
