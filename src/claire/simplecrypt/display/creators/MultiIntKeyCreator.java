...
public abstract class MultiIntKeyCreator<Key extends ISecret<?>>
	...
	protected MultiIntKeyCreator(String[] methods, String entry)
	{
		combo = new JComboBox<String>(methods);
		this.entry = entry;
	}
	...
	protected abstract Key extract(String phrase);
	protected abstract Key extract(int[] arr);
						 ...
						 implements ActionListener
	{
		...
		private final MultiIntKeyCreator<?> owner;
		private final int pos;
		
		public ShiftField(MultiIntKeyCreator<?> owner, int pos)
		{
			...
			gbc.gridy = 0;
			gbc.gridx = 0;
			this.add(new JLabel(owner.entry), gbc);
			gbc.gridx++;
			gbc.weightx = 0.1D;
			this.add(shift, gbc);
